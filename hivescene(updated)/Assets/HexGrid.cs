    ...
    void SpawnCell(int q, int r, CellType type)
{
    ...
    if (hexSprite != null)
    {
        ...
        // ADD THIS — without a collider, mouse clicks pass straight through
        var col = go.AddComponent<PolygonCollider2D>();
        // Build a hexagon shape matching the sprite size
        float s = hexSize * 0.95f;   // slight inset so gaps between hexes don't mis-click
        col.pathCount = 1;
        ...
        // Tag it so HandleInput can raycast cleanly
    ...
    void HandleInput()
{
    ...
    // Number keys to select cell type
    ...
    // Raycast against the polygon colliders we added
    ...
    // Find which cell was hit by matching the GameObject name
    Vector2Int coords = WorldToAxial(hit.collider.transform.position);
    ...
    void PlaceType(HexCell cell)
{
    ...
    ComboDetector.Instance?.CheckCombos(cell.AxialCoords);
    AudioManager.Instance?.PlayPlaceCell();   // satisfying click on every place
